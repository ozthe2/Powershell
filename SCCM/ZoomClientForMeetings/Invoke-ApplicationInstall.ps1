    ...
    Process { 
        ...
        if ($InstallBasedOnOfficeBitness) {
            ...
            switch ($obj.OSIs64Bit) {
                $True {Start-Process 'C:\Windows\System32\msiexec.exe' " /i ""$workingdir\ZoomInstallerFull.msi"" /qn /norestart" -NoNewWindow -Wait}
                default {Start-Process 'C:\Windows\System32\msiexec.exe' " /i ""$workingdir\ZoomInstallerFull.msi"" /qn /norestart" -NoNewWindow -Wait}
            }
        }
