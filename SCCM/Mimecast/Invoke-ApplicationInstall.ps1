...
Function Invoke-PreInstallation {   
    # Force close Outlook with no warning if it's open
    ...
    Process { 
        ...
        if ($InstallBasedOnOfficeBitness) {
            ...
            switch ($Obj.OfficeIs32Bit) {
                $true {Start-Process 'C:\Windows\System32\msiexec.exe' " /i ""$workingdir\Mimecast for outlook 7.8.0.125 (x86).msi"" /qn" -NoNewWindow -Wait}
                $false {Start-Process 'C:\Windows\System32\msiexec.exe' " /i ""$workingdir\Mimecast for outlook 7.8.0.125 (x64).msi"" /qn" -NoNewWindow -Wait }
