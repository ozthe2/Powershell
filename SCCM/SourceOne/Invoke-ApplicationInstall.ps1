    ...
    Process {
        ...
        if ($InstallBasedOnOfficeBitness) {
            ...
            switch ($Obj.OfficeIs32Bit) {
                $true {start-process -FilePath ".\7.27.7007\ES1_OfflineAccess.exe" -ArgumentList "/S /v/qn" -NoNewWindow -Wait;start-process -FilePath ".\Hotfix\ES1_OfflineAccess.exe" -ArgumentList "/S /v/qn" -NoNewWindow -Wait}
                $false {start-process -FilePath ".\7.27.7007\ES1_OfflineAccessx64.exe" -ArgumentList "/S /v/qn" -NoNewWindow -Wait;start-process -FilePath ".\Hotfix\ES1_OfflineAccessx64.exe" -ArgumentList "/S /v/qn" -NoNewWindow -Wait}
            ...
            switch ($obj.OSIs64Bit) {
                $True {"Install a 64-bit application here"}
                default {"Install a 32-bit application here"}
            }
        }
